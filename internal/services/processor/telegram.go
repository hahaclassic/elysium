...
import (
	...
	postgres "github.com/hahaclassic/golang-telegram-bot.git/internal/storage/postgresql"
...
type Session struct {
	operationHistory []model.Operation
...
func New(config *Config) (*Processor, error) {
	...
	// 	return nil, err
	// }
	storageConf, err := postgres.NewConfig()
	if err != nil {
		return nil, err
	}
	storage, err := postgres.NewStorage(context.TODO(), storageConf)
	if err != nil {
		return nil, err
	}

	processor := &Processor{
		tg:          tgclient.New(config.TgHost, config.MainToken),
		logger:      tgclient.New(config.TgHost, config.LoggerToken),
